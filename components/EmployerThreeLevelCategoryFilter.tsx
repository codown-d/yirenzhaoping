...
export default function EmployerThreeLevelCategoryFilter({ 
  selectedCategory = "frontend",
  selectedSpecialties = [],
  onCategoryChange,
  onSpecialtiesChange,
  className = ""
}: EmployerThreeLevelCategoryFilterProps) {
  const [activeCategory, setActiveCategory] = useState<string>(selectedCategory)
  const [expandedSubcategories, setExpandedSubcategories] = useState<Set<string>>(new Set())
  const [selectedTypes, setSelectedTypes] = useState<string[]>(selectedSpecialties)
  ...
  useEffect(() => {
    setSelectedTypes(selectedSpecialties)
  }, [selectedSpecialties])
  ...
  const handleSpecialtyToggle = (specialty: string) => {
    const newSelectedTypes = selectedTypes.includes(specialty)
      ? selectedTypes.filter(type => type !== specialty)
      : [...selectedTypes, specialty]
    
    setSelectedTypes(newSelectedTypes)
    onSpecialtiesChange?.(newSelectedTypes)
  }

  // 移除选中的专业
  const removeSpecialty = (specialty: string) => {
    const newSelectedTypes = selectedTypes.filter(type => type !== specialty)
    setSelectedTypes(newSelectedTypes)
    onSpecialtiesChange?.(newSelectedTypes)
  }
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {selectedTypes.length > 0 && (
        <div className="bg-white rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-medium">已选择专业</h3>
      ...
      {/* 二级和三级分类筛选 */}
      <div className="bg-white rounded-2xl p-4">
        <h3 className="text-base font-medium mb-3">选择专业领域</h3>
                      ...
                      {subcategory.children.map((item) => (
                        ...
                        >
                          <Checkbox
                            ...
                            onCheckedChange={() => handleSpecialtyToggle(item.name)}
                          />
                          <Label 
